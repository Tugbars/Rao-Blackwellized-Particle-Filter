 ...
 * @brief Test/Benchmark: RBPF-KSC + Sleeping Storvik Integration
 *
 * Tests:
 *   1. Initialization and configuration
 *   2. Particle info extraction
 *   3. Storvik sync during resample
 *   4. Parameter convergence comparison (Liu-West vs Storvik)
 *   5. Latency benchmark
 *
 * Build (requires MKL):
...
/*═══════════════════════════════════════════════════════════════════════════
 * TEST 6: Per-Particle Params Active in Storvik Mode (Regression Test)
 *
 * This test catches the critical bug where liu_west.enabled == 0 in
 * Storvik mode, causing the predict step to ignore particle_mu_vol.
 *═══════════════════════════════════════════════════════════════════════════*/

static void test_per_particle_params_active(void) {
    printf("═══ Test 6: Per-Particle Params Active (Regression) ═══\n");
    
    RBPF_Extended *ext = rbpf_ext_create(100, 4, RBPF_PARAM_STORVIK);
    
    /* CRITICAL CHECK: liu_west.enabled must be 1 for predict to use particle_mu_vol */
    ASSERT(ext->rbpf->liu_west.enabled == 1, 
           "liu_west.enabled == 1 in Storvik mode (activates per-particle params)");
    
    /* But the learning logic should be disabled (Storvik controls values) */
    ASSERT(ext->rbpf->liu_west.learn_mu_vol == 0,
           "Liu-West internal learning disabled (Storvik overwrites)");
