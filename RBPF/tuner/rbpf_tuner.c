...
TunerConfig tuner_config_defaults(void)
{
    ...
    /* Asymmetric gap control */
    cfg.use_asymmetric_gaps = 0; /* Disabled by default */
    cfg.mu_vol_r3_max = -0.3f;   /* R3 max: ~75% daily vol */
    cfg.gap_scale_high = 0.8f;   /* R2→R3 gap is 80% of base gap */
...
TunerConfig tuner_config_balanced(void)
{
    ...
    /* Enable asymmetric gaps to keep R3 realistic */
    cfg.use_asymmetric_gaps = 1;
    cfg.mu_vol_r3_max = -0.4f;  /* R3 max: ~67% daily vol, realistic for crisis */
    cfg.gap_scale_high = 0.70f; /* R2→R3 gap is 70% of base gap */

    /* Search range that includes realistic configurations */
    cfg.mu_vol_base = -4.8f; /* Slightly higher base for R0 */
    cfg.mu_vol_gap_min = 1.2f;
    cfg.mu_vol_gap_max = 1.7f; /* Don't go too wide */
    cfg.mu_vol_gap_steps = 6;

    /* NO middle-regime bonus - we want balance */
    cfg.weight_middle_regimes = 0.0f;

    /* Equal weight on overall accuracy */
    cfg.weight_accuracy = 1.0f;

    /* Slightly more sticky */
    cfg.self_trans_min = 0.975f;
    cfg.self_trans_max = 0.995f;
    cfg.self_trans_steps = 5;

    /* More eval runs for stability */
    cfg.n_eval_runs = 2;
...
float tuner_evaluate(RBPFTuner *tuner, TunerResult *result)
{
    ...
    for (int r = 0; r < n_regimes; r++)
    {
        result->regime_accuracy[r] = total[r] > 0 ? (float)correct[r] / total[r] : 0.0f;
        total_correct += correct[r];
        total_samples += total[r];

        for (int j = 0; j < n_regimes; j++)
        {
            result->confusion[r][j] = total[r] > 0 ? (float)confusion[r][j] / total[r] : 0.0f;
        }
    }

    result->overall_accuracy = total_samples > 0 ? (float)total_correct / total_samples : 0.0f;
    result->n_switches = n_switches;
    result->switch_rate = total_samples > 0 ? (float)n_switches / total_samples : 0.0f;
    ...
    if (n_regimes >= 4 && cfg->weight_middle_regimes > 0)
    {
        float middle_acc = (result->regime_accuracy[1] + result->regime_accuracy[2]) / 2.0f;
        obj += cfg->weight_middle_regimes * middle_acc;
    }

    /* Penalty for ANY regime below threshold (prevents sacrificing extremes) */
    float min_acc = 1.0f;
    for (int r = 0; r < n_regimes; r++)
    {
        if (result->regime_accuracy[r] < min_acc)
        {
            min_acc = result->regime_accuracy[r];
        }
    }
    float min_thresh = 0.62f; /* No regime should fall below 62% */
    if (min_acc < min_thresh)
    {
        obj -= (min_thresh - min_acc) * 4.0f; /* Very strong penalty */
    }

    /* Additional penalty for imbalance between regimes */
    float max_acc = 0.0f;
    for (int r = 0; r < n_regimes; r++)
    {
        if (result->regime_accuracy[r] > max_acc)
        {
            max_acc = result->regime_accuracy[r];
        }
    }
    float imbalance = max_acc - min_acc;
    if (imbalance > 0.15f)
    { /* Penalize if any regime is >15% better than worst */
        obj -= (imbalance - 0.15f) * 2.0f;
    }
...
float tuner_grid_search(RBPFTuner *tuner)
{
    ...
    for (int g = 0; g < cfg->mu_vol_gap_steps; g++)
    {
        ...
        /* Set mu_vol with this gap (with optional asymmetric scaling) */
        result.mu_vol[0] = cfg->mu_vol_base;
        for (int r = 1; r < n_regimes; r++)
        {
            float this_gap = gap;

            /* Asymmetric gap: tighter for high regimes to keep R3 realistic */
            if (cfg->use_asymmetric_gaps && r == n_regimes - 1)
            {
                this_gap = gap * cfg->gap_scale_high;
            }

            result.mu_vol[r] = result.mu_vol[r - 1] + this_gap;

            /* Clamp R3 to max if specified */
            if (cfg->use_asymmetric_gaps && r == n_regimes - 1)
            {
                if (result.mu_vol[r] > cfg->mu_vol_r3_max)
                {
                    result.mu_vol[r] = cfg->mu_vol_r3_max;
                }
            }
        }
        ...
        for (int s = 0; s < cfg->self_trans_steps; s++)
        {
            ...
            for (int h = 0; h < cfg->hold_steps; h++)
            {
                ...
                for (int p = 0; p < cfg->prob_thresh_steps; p++)
                {
                    ...
                    memset(avg_result.confusion, 0, sizeof(avg_result.confusion)); /* FIX: zero confusion matrix */
                    ...
                    if (avg_result.objective > tuner->best.objective)
                    {
                        ...
                        if (cfg->verbose >= 1)
                        {
                            ...
                            if (n_regimes >= 4)
                            {
                                printf("[R0=%.0f%% R1=%.0f%% R2=%.0f%% R3=%.0f%%]",
                                       avg_result.regime_accuracy[0] * 100,
                                       avg_result.regime_accuracy[1] * 100,
                                       avg_result.regime_accuracy[2] * 100,
                                       avg_result.regime_accuracy[3] * 100);
                            }
                            printf("\n");
...
{
    if (!result || !buffer || buffer_size < 512)
        return;
    ...
    for (int i = 0; i < n_regimes; i++)
    {
        ...
        for (int j = 0; j < n_regimes; j++)
        {
            float val;
            if (i == j)
                val = result->self_trans;
            else if (abs(i - j) == 1)
            {
                if (i == 0 || i == n_regimes - 1)
                    val = 1.0f - result->self_trans;
                else
                    val = off_diag;
            }
            else
